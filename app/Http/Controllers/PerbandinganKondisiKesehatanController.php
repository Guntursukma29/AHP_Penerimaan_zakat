...
class PerbandinganKondisiKesehatanController extends Controller
{
    public function index()
    {
        ...
        $this->fillMatrixAndColumnTotals($perbandingankondisikesehatan, $matrix, $columnTotals);
        ...
        $perbandinganArray = $this->getPerbandinganArray($perbandingankondisikesehatan);

        return view('p_alternatif.kondisikesehatan', compact('penerimaanzakat', 'matrix', 'columnTotals', 'normalizedMatrix', 'eigenVector', 'lambdaMax', 'ci', 'cr', 'sumEigenVector', 'perbandinganArray', 'kriteria'));
    }
    ...
    private function fillMatrixAndColumnTotals($perbandingankondisikesehatan, &$matrix, &$columnTotals)
    {
        foreach ($perbandingankondisikesehatan as $perbandingan) {
    ...
    private function getPerbandinganArray($perbandingankondisikesehatan)
    {
        $perbandinganArray = [];
        foreach ($perbandingankondisikesehatan as $perbandingan) {
